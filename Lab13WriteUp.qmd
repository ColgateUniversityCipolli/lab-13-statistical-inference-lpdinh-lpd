...
```{r}
t.test(x=dat.birds$closer_vals,
...
library(effectsize)
hedges_g(x=dat.birds$further_vals, mu = 0)
```

* The difference between populations was statistically significant ($t=9.14$, $p<0.0001$; $g=1.77$; 95% CI: 0.30, 0.48).

```{r}
t.test(x=dat.birds$difference,
      mu=0, alternative="two.sided")

library(effectsize)
hedges_g(x=dat.birds$difference, mu = 0)
```
...
ggplot(ggdat) +
  ...
  scale_fill_manual(
    ...
    c("Null Distribution" = "salmon", 
       "Resampling Distribution" = "teal")
    )+
  labs(y = "Density", fill="") +
